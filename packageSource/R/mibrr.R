                  ...
                  optControl = list())
{
    ...
    if(require(rlecuyer)) {
        if(gibbsControl$createRngStream) {# Initialized a rlecuyer stream object
            .lec.SetPackageSeed(rep(gibbsControl$rngSeed, 6))
            .lec.CreateStream("myStream")
            oldRng <- .lec.CurrentStream("myStream")
            currentStreamName <- "myStream"
        } else {# Use an extant rlecuyer stream object
            oldRng <- .lec.CurrentStream(gibbsControl$streamName)
            currentStreamName <- gibbsControl$streamName
        }
        useLecuyerRNG <- TRUE
    } else {# Use R's default Mersenne-Twister
        set.seed(gibbsControl$rngSeed,
    ...
    ## When no targets are given, all incomplete variables not listed
    ## in 'ignoreVariables' are imputed.
    if(is.null(targetVariables)) {
        targetCandidates <-
            colnames(rawData)[!colnames(rawData) %in% ignoreVariables]
        warning(paste0("\nYou didn't specify any target variables,\n",
                       "so I'm going to impute the missingness of every\n",
                       "variable in 'rawData' that is not listed in ",
                       "'ignoreVariables'.\n")
                )
    ...
    if(is.null(missCode)){
        completeTargets <-
            colMeans(is.na(rawData[ , targetCandidates])) == 0
        if(all(completeTargets)) {
            stop(paste0("Your target variables appear ",
                        "to be fully observed.\n",
                        "Did you forget to provide ",
                        "a value for 'missCode'?\n")
                 )
        }
    } else {
    ...
    if(any(completeTargets)) {
        warning(paste0("\nThe potential target variables {",
                       paste(targetCandidates[completeTargets],
                             collapse = ", "),
                       "} are fully observed,\n",
                       "so they will not be imputed.\n")
                )
    }
    ...
    if(doMiben) {
        ...
        if(is.null(optControl$lambdaStarts)) {
            if(optControl$usePCStarts) {
                ## Important to call this before the NA's
                ## are replaced by missCode:
            ...
            options(warn = -1)
            lambdaVec <- as.vector(
                matrix(optControl$lambdaStarts, nTargets, 1)
            )
            options(warn = 0)
            lambdaMat <- cbind(lambdaVec, 0)
        }
    ...
    if(is.null(missCode)) {
        if(max(abs(centeredData), na.rm = TRUE) < 1.0) {
            ...
            codeMag <-
                floor(
                    log10(
                        max(abs(centeredData), na.rm = TRUE)
                    )
                ) + 2
    ...
    gibbsOut <-
        runGibbs(inData = centeredData,
                 ...
                 emConvTol = ifelse(doMiben, optControl$emEpsilon, -1),
                 lambdaWindow = optControl$smoothingWindow,
                 verboseIters = verboseIters,
                 verboseErrors = verboseErrors,
                 doMibl = !doMiben)
    
    ## Compute the potential scale reduction factors (R-Hats)
    ## for the posterior imputation model parameters:
    ...
    ## Draw the imputations from the convergent
    ...
    if(returnModelParams) {
        ...
        for(j in 1 : length(gibbsOut)) {
            ...
            lamList[[j]] <-
                gibbsOut[[j]]$lambdaHistory[totalEmIters, ]
        }
        outList$params <- list(
            beta = betaList,
            tau = tauList,
...
### Specify a wrapper function to implement Multiple
...
### Specify a wrapper function to implement Multiple
### Imputation with the Bayesian Lasso (MIBL):
