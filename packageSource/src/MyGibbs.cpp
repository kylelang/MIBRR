...
{ 
  ...
  if(_useElasticNet) {
    ...
    outLam = VectorXd::Constant(1, _lambdas[0]);
  }
  return outLam; 
}
...
{  
  ...
  if(_useElasticNet) {
    _lambdas = pooledLambdas.transpose();
  }
  else {
    _lambdas[0] = pooledLambdas[0];
  }
}
...
{
  _betas = _betaSam.bottomRows(_betaSam.rows() - 1)
    .colwise().mean().transpose();
  _taus = _tauSam.bottomRows(_tauSam.rows() - 1)
    .colwise().mean().transpose();
  _sigma = _sigmaSam.tail(_sigmaSam.size() - 1).mean();
  
  VectorXd meanImps = 
    _impSam.bottomRows(_impSam.rows() - 1).colwise().mean();
...
void MyGibbs::updateTaus(MyData &myData)
{
  ...
  try {
    ...
    if(_useElasticNet) {// Miben Version
      ArrayXd tauMeansNumerator = 
	ArrayXd::Constant(nPreds, lambda1);
	...
	( 2.0 * lambda2 * _betas.tail(nPreds).array().abs() );
      ...
      double tauMeansNumerator = 
	pow(lambda1, 2) * _sigma;
      
      tauMeans = 
	( tauMeansNumerator / 
	  _betas.tail(nPreds).array().square() ).sqrt();
  ...
  ArrayXd newTaus;
  if(_useElasticNet) {// MIBEN Version
    newTaus = (tmpDraws + 1.0) / tmpDraws;    
  }
  else {// MIBL Version
    newTaus = 1.0 / tmpDraws;
  }
  ...
  if(_storeGibbsSamples)
    _tauSam.row(_drawNum) = newTaus.transpose();
}// END updateTaus ()
...
void MyGibbs::updateBetas(MyData &myData)
{
  ...
  try {
    ...
    // of the IV's penalized crossproducts:
    aMatrixCholesky.compute(aMatrix);
    
    tmpMat = _sigma * MatrixXd::Identity(nPreds, nPreds);
    
    betaMeans = 
      aMatrixCholesky.solve( myData.getIVs(_targetIndex).transpose() * 
  ...
  if(_storeGibbsSamples)
    _betaSam.row(_drawNum) = newBetas.transpose();
}// END updateBetas ()
...
void MyGibbs::updateSigma(MyData &myData)
{
  ...
  if(_useElasticNet) {// MIBEN Version
    double scaleSum = ( 
		       _taus / (_taus - 1.0) * 
		       _betas.tail(nPreds).array().square() 
			).sum();
    
    double sigmaShape = (double(nObs) / 2.0) + double(nPreds);
    double sigmaScale = 0.5 * ( 
			       sse + lambda2 * scaleSum + 
			       ( pow(lambda1, 2) / (4.0 * lambda2) ) * 
			       _taus.sum() 
				);
    ...
    while(!isDrawValid) {// Rejection sampling to draw a Sigma variate
      ...
      double uiGammaShape = 
	pow(lambda1, 2) / (8.0 * testDraw * lambda2);
      double uiGammaDraw = myData.calcIncGamma(0.5, uiGammaShape, false);
      isDrawValid = 
	log(thresholdDraw) <= ( double(nPreds) * log( tgamma(0.5) ) ) - 
    ...
    double penaltyTerm = _betas.tail(nPreds).transpose() * 
      tmpMat * _betas.tail(nPreds);
    
    double sigmaShape = 
      0.5 * ( (double(nObs) - 1.0) + double(nPreds) );
...
void MyGibbs::updateImputations(MyData &myData)
{
  int nObs = myData.nObs();
  int nPreds = myData.nPreds();
  ArrayXb nonresponseVector = 
    myData.getNonresponseVector(_targetIndex);
  VectorXd tmpBiasVector = VectorXd::Ones(nObs);
  ...
  for(int i = 0; i < nObs; i++) {
    errorVector[i] = R::rnorm(0.0, tmpSd);
  }
  
  // Draw a vector of imputations from the posterior predictive
  ...
  // Replace the missingness in the target variable with the 
  ...
  if(_storeGibbsSamples)
    _impSam.row(_drawNum) = tmpImps.transpose();
				    ...
				    void *extraOptData)
{
  ...
  double w8 = ( nPreds * pow(lambdaVec[0], 2) ) / 
    ( 8.0 * pow(lambdaVec[1], 2) );
  ...
  ArrayXd term2 = ( ( _tauSam / (_tauSam - 1) ) * 
		    _betaSam.rightCols(nPreds).array().square()
		    ).rowwise().sum();  
  // END compute LL and gradient terms.

  if( !gradVec.empty() ) {// Calculate the gradient vector:
    ArrayXd term3 = (1 / uiGammaArray) * ( 1 / w4.sqrt() ) * 
      (-1.0 * w4).exp() * (1 / _sigmaSam.array());
    gradVec[0] = ( w5 + (w6 * term3) -
		   w7 * _tauSam.rowwise().sum() ).mean();
    gradVec[1] = ( -1.0 * (w8 * term3) - (w3 * term2) +
		   w9 * _tauSam.rowwise().sum() ).mean();
  }
  
  // Calculate the objective value:
  double condLL = ( w1 - (nPreds * term1) - (w2 * term2) -
		    ( w4 * _tauSam.rowwise().sum() ) ).mean();
  return condLL;// Return the conditional loglikelihood value
}// END eNetLambdaObjective()


// As suggested by the nlopt authors, specify a simple  wrapper function 
// for eNetLambdaObjective() so that nlopt will run inside the MyGibbs 
// class without a fuss:
...
void MyGibbs::optimizeMibenLambdas(bool preOptimize) 
{
  ...
  while(!optimized) {//Try different algorithms until convergence
	...
	lambdaVec[0] = _lambdas[0];
	lambdaVec[1] = _lambdas[1];// Reset the lambdas
	if(optMethod > 3) {
	  if(_verboseErrors) {
...
void MyGibbs::updateLambdas()
{
  if(_useElasticNet) {// MIBEN version
    ...
    // 1) A rough, gradient-free, pre-optimization is done to move the 
    //    estimates into the neighborhood of the MLE.
    // 2) Gradient-based optimization is used to fine-tune the estimates
    //    produced by Step (1).
    ...
    // For MIBL, optimization is done via the closed-form 
    // update rule given by Park and Casella (2008).
    double lambdaDenominator = 
      ( _tauSam.colwise().mean() ).sum();
    double newLambda = 
      sqrt( ( 2.0 * double(nPreds) ) / lambdaDenominator );
