	       ...
	       double missingDataCode) 
{
  ...
  _nonresponseFilter = ArrayXXb::Constant(_myData.rows(),
					  _myData.cols(),
					  false);
...
{ 
  ...
  MatrixXd tmpMat = _myData * _dataScales.asDiagonal().inverse(); 
...
void MyData::computeNonresponseFilter() 
{
  ...
  VectorXd nObsVec = VectorXd::Constant(_myData.cols(), _myData.rows()).array();
...
void MyData::fillMissing(int nTargets)
{
  ...
  for(int i = 0; i < nObs; i++) {
    for(int j = 0; j < nVars; j++) {
      if(_nonresponseFilter(i, j)) _myData(i, j) = 0.0;
    }
  }
  ...
  if(nTargets > 1) {
	...
	if( _nonresponseFilter(i, j) ) _myData(i, j) = mvnDraws(i, j);
      }
    }  
  }
  else {
...
void MyData::fillMissing(MatrixXd &newTargets)
{
  ...
  for(int i = 0; i < nObs; i++) {
    for(int j = 0; j < nTargets; j++) {
      if(_nonresponseFilter(i, j)) _myData(i, j) = newTargets(i, j);
    }
  }
